...
is( join('', @a), '369', 'check results for array' );

@a = mce_grep \&_task, [ 1..9 ];
is( join('', @a), '369', 'check results for array ref' );

@a = mce_grep_f \&_task, $in_file;
is( join('', @a), '369', 'check results for path' );

@a = mce_grep_f \&_task, $fh_data;
is( join('', @a), '369', 'check results for glob' );
