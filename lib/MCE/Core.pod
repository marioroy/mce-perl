...
This document describes MCE::Core version 1.815
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      use_slurpio => 1,                   ## Default 0
         ...
         # Not all modules on CPAN are thread-safe nor safe to use with
         # many processes. Set posix_exit explicitly if your application
         # is crashing, due to using a module with a DESTROY or END block
         # not accounting for the process ID the object was constructed
         # under: e.g. Cache::BDB.
