...
This document describes MCE::Core version 1.699_001
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      chunk_size => 2000,                 ## Default $MCE::CHUNK_SIZE
         ...
         # before calling user_func. A value above 64M will change
         # to 64M quietly (the maximum allowed).
      ...
      mutex_type  => 'channel',           ## Default 'auto'
      mutex_type  => 'flock',

         # MCE 1.6 and below performed IPC locking via Fcntl flock.
         # The thing about file locking is that it requires one file
         # handle per each data channel times the number of workers.
         # Channel locking in 1.7 resolves this. Thus, allows more
         # threads when running with threads. Note: choosing 'flock'
         # has no effect on Cygwin.

         # Auto disables locking when the number of workers is lesser
         # or equal to the number of data channels. Otherwise, MCE
         # enables channel locking for data channels shared by 2 or
         # more workers. The behavior is similar as in MCE 1.6.

         # Specify 'channel' to allow processes and threads, spawned
         # by MCE workers, to call any MCE method without additional
         # synchronization at the application level. This enforces
         # locking regardless on how many MCE workers.
      ...
      use_slurpio => 1,                   ## Default 0
         ...
         # blades with input coming from shared storage.

         # Set posix_exit to avoid END and destructor processing
         # for non-threads spawned via fork.

         # Enable slurpio to pass the raw chunk (scalar ref)
         # to the user function when reading input files.
      ...
      on_post_run => \&on_post_run,       ## Default undef
         ...
         # Execute code block after a worker exits.
         # exit, MCE->exit, or die

         # Execute code block after running a job.
...
L<MCE::Candy|MCE::Candy>. Stubs exist in MCE which load MCE::Candy
automatically.
...
calling this method. See L<MCE::Flow|MCE::Flow> and L<MCE::Loop|MCE::Loop>
for additional use-case.
