...
This document describes MCE::Core version 1.813
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      chunk_size => 2000,                 ## Default 1
         ...
         # before calling user_func. A value above 64 MiB will change
         # to 64 MiB quietly (the maximum allowed).
      ...
      use_slurpio => 1,                   ## Default 0
         ...
         # Set posix_exit to avoid all END and destructor processing for
         # non-threads spawned via fork. Constructing MCE inside a thread
         # implies posix_exit => 1 or if present CGI.pm, FCGI.pm, Tk,
         # Gearman::Util, Gearman::XS, or Mojo::IOLoop.
      ...
      progress => sub { ... },            ## Default undef
         ...
         # See section labeled "PROGRESS INFO" for demonstrations.
...
The max_retries option, added in 1.7, causes MCE to retry a failed chunk from
a worker dying while processing input data or sequence of numbers.

MCE configures the on_post_exit option with the following code in the absence
of on_post_exit. Otherwise, simply tailor this into your on_post_exit code.

   ## What is done behind the scene when max_retries is specified and
   ## omitting the on_post_exit option in MCE.
   ...
   sub user_func {
      my ($mce, $chunk_ref, $chunk_id) = @_;
      die "Died : chunk_id == 3" if $chunk_id == 3;
      print "$mce->{_wid}: $chunk_id\n";
   }

   my $mce = MCE->new(
      max_workers => 1, user_func => \&user_func,
      max_retries => 2,
   )->spawn;

   my @input_data; push @input_data, qw( 0 1 2 3 4 5 6 7 );
   ...
   1: 1
   1: 2
   Died : chunk_id == 3 at script.pl line 8, <__ANONIO__> line 6.
   Error: Chunk 3 failed, retrying chunk attempt #1
   Died : chunk_id == 3 at script.pl line 8, <__ANONIO__> line 7.
   Error: Chunk 3 failed, retrying chunk attempt #2
   Died : chunk_id == 3 at script.pl line 8, <__ANONIO__> line 14.
...
Next is the code using L<MCE::Flow> to do the same thing.
   ...
   MCE::Flow->run( sub { ... }, \@input );
   MCE::Flow->finish();

The two examples produce the following output.
...
For sequence of numbers, via the C<sequence> option, one must account for
C<step_size>, typically C<1> when not specified.
...
Here is the same thing using L<MCE::Flow>, a MCE Model.
   ...
   MCE::Flow->run_seq( sub { ... }, 1, 100, 2 );
   MCE::Flow->finish();

The manager-process outputs the following for the two prior examples.
...
Changing the C<chunk_size> option to C<1> means workers notify the manager
process more often, thus increasing granularity. Take a look at the output.
...
For consistency, here is the example using L<MCE::Flow>.
